...
async function execute(
  ...
  const hasPermission =
    ...
    ticket.panel.supportRoleIDs
      .split(",")
      .some((roleId) => interaction.member?.roles.includes(roleId.trim()));
  ...
  const transferTicketID = parseInt(
    interaction.options.getString("transferID", true)
  );

  if (!isNaN(transferTicketID)) {
    return interaction.createMessage({
      ...interaction.getErrorReply(
